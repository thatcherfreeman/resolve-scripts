-- IMOPORTANT:
-- Replace below with the path of your exiftool installation
local is_windows = package.config:sub(1, 1) ~= "/"
local exiftool_path = '/opt/homebrew/bin/exiftool'
if is_windows then
...
function bradford_chromatic_adaptation(source_xy, destination_xy)
    local bradford = {0.8951000, 0.2664000, -0.1614000, -0.7502000, 1.7135000, 0.0367000, 0.0389000, -0.0685000,
                      1.0296000}
    ...
    local out = multiply(inverse(bradford),
        multiply(diagonal({destination_lms[1] / source_lms[1], destination_lms[2] / source_lms[2],
                           destination_lms[3] / source_lms[3]}), bradford))
    return out
end
...
function get_XYZ_to_rgb_matrix(primaries)
    ...
    local xyzmat = {red_XYZ[1], green_XYZ[1], blue_XYZ[1], red_XYZ[2], green_XYZ[2], blue_XYZ[2], red_XYZ[3],
                    green_XYZ[3], blue_XYZ[3]}
...
function illuminant_to_temperature(illuminant)
    ...
    elseif illuminant == "D50" then
        return 5000.0
    elseif illuminant == "D55" or illuminant == "Daylight" or illuminant == "Fine Weather" or illuminant == "Flash" or
        illuminant == "Standard Light B" then
        return 5500.0
...
function xy_to_temperature(xy)
    ...
    local rt = { -- reciprocal temperature (K)
    0.0, 10.0e-6, 20.0e-6, 30.0e-6, 40.0e-6, 50.0e-6, 60.0e-6, 70.0e-6, 80.0e-6, 90.0e-6, 100.0e-6, 125.0e-6, 150.0e-6,
    175.0e-6, 200.0e-6, 225.0e-6, 250.0e-6, 275.0e-6, 300.0e-6, 325.0e-6, 350.0e-6, 375.0e-6, 400.0e-6, 425.0e-6,
    450.0e-6, 475.0e-6, 500.0e-6, 525.0e-6, 550.0e-6, 575.0e-6, 600.0e-6}

    local uvt = {{0.18006, 0.26352, -0.24341}, {0.18066, 0.26589, -0.25479}, {0.18133, 0.26846, -0.26876},
                 ...
                 {0.24010, 0.34308, -2.0637}, {0.24792, 0.34655, -2.4681}, -- Note: 0.24792 is a corrected value for the error found in W&S as 0.24702
                 {0.25591, 0.34951, -2.9641}, {0.26400, 0.35200, -3.5814}, {0.27218, 0.35407, -4.3633},
                 {0.28039, 0.35577, -5.3762}, {0.28863, 0.35714, -6.7262}, {0.29685, 0.35823, -8.5955},
                 {0.30505, 0.35907, -11.324}, {0.31320, 0.35968, -15.628}, {0.32129, 0.36011, -23.325},
                 {0.32931, 0.36038, -40.770}, {0.33724, 0.36051, -116.45}}
...
function get_xyz_to_camera_matrix_no_fm(white_xy, exif_tags, override_wb)
    ...
    if exif_tags['CalibrationIlluminant1'] ~= nil and exif_tags['CalibrationIlluminant2'] ~= nil and
        exif_tags['CalibrationIlluminant1'] ~= exif_tags['CalibrationIlluminant2'] then
        num_illuminants = 2
    end
    local camera_matrix, camera_calibration, analog_balance, forward_matrix
    if num_illuminants == 1 or override_wb == exif_tags["CalibrationIlluminant1"] then
        camera_matrix = multiply(multiply(diagonal(exif_tags['AnalogBalance']), exif_tags['CameraCalibration1']),
            exif_tags['ColorMatrix1'])
    elseif num_illuminants == 2 and override_wb == exif_tags["CalibrationIlluminant2"] then
        camera_matrix = multiply(multiply(diagonal(exif_tags['AnalogBalance']), exif_tags['CameraCalibration2']),
            exif_tags['ColorMatrix2'])
    elseif num_illuminants == 2 and override_wb == "Current WB" then
        ...
        local camera_matrix1 = multiply(multiply(diagonal(exif_tags['AnalogBalance']), exif_tags['CameraCalibration1']),
            exif_tags['ColorMatrix1'])
        local camera_matrix2 = multiply(multiply(diagonal(exif_tags['AnalogBalance']), exif_tags['CameraCalibration2']),
            exif_tags['ColorMatrix2'])
        camera_matrix = interpolate_matrix(camera_matrix1, camera_matrix2, illuminant1_temp, illuminant2_temp,
            white_temp)
    else
...
function get_xyz_to_camera_matrix_with_fm(white_xy, exif_tags, override_wb)
    ...
    local camera_matrix, camera_calibration, analog_balance, forward_matrix
    if num_illuminants == 1 or override_wb == exif_tags["CalibrationIlluminant1"] then
        local reference_neutral = multiply(inverse(multiply(diagonal(exif_tags['AnalogBalance']),
            exif_tags['CameraCalibration1'])), exif_tags["AsShotNeutral"])
        local dmatrix = inverse(diagonal(reference_neutral))
        camera_matrix = multiply(exif_tags["ForwardMatrix1"], multiply(dmatrix, inverse(
            multiply(diagonal(exif_tags['AnalogBalance']), exif_tags['CameraCalibration1']))))
    elseif num_illuminants == 2 and override_wb == exif_tags["CalibrationIlluminant2"] then
        local reference_neutral = multiply(inverse(multiply(diagonal(exif_tags['AnalogBalance']),
            exif_tags['CameraCalibration2'])), exif_tags["AsShotNeutral"])
        local dmatrix = inverse(diagonal(reference_neutral))
        camera_matrix = multiply(exif_tags["ForwardMatrix2"], multiply(dmatrix, inverse(
            multiply(diagonal(exif_tags['AnalogBalance']), exif_tags['CameraCalibration2']))))
    elseif num_illuminants == 2 and override_wb == "Current WB" then
        local illuminant1_temp = illuminant_to_temperature(exif_tags['CalibrationIlluminant1'])
        local illuminant2_temp = illuminant_to_temperature(exif_tags['CalibrationIlluminant2'])
        local white_temp = xy_to_temperature(white_xy)

        local reference_neutral1 = multiply(inverse(multiply(diagonal(exif_tags['AnalogBalance']),
            exif_tags['CameraCalibration1'])), exif_tags["AsShotNeutral"])
        local reference_neutral2 = multiply(inverse(multiply(diagonal(exif_tags['AnalogBalance']),
            exif_tags['CameraCalibration2'])), exif_tags["AsShotNeutral"])
        local dmatrix1 = inverse(diagonal(reference_neutral1))
        local dmatrix2 = inverse(diagonal(reference_neutral2))
        local camera_matrix1 = multiply(exif_tags["ForwardMatrix1"], multiply(dmatrix1, inverse(
            multiply(diagonal(exif_tags['AnalogBalance']), exif_tags['CameraCalibration1']))))
        local camera_matrix2 = multiply(exif_tags["ForwardMatrix2"], multiply(dmatrix2, inverse(
            multiply(diagonal(exif_tags['AnalogBalance']), exif_tags['CameraCalibration2']))))
...
function neutral_to_xy(neutral, exif_tags)
    ...
    for pass = 1, max_passes do
        local xyz_to_camera = get_xyz_to_camera_matrix_no_fm(last_xy, exif_tags)
        local next_xy = xyY_to_xy(XYZ_to_xyY(multiply(inverse(xyz_to_camera), neutral)))
        if (math.abs(next_xy[1] - last_xy[1]) + math.abs(next_xy[2] - last_xy[2])) < 0.0000001 then
            return next_xy
        end
...
win = disp:AddWindow({
    ID = "MyWin",
    WindowTitle = "DNG Matrix Calculator",
    ...
    ui:VGroup{
        ...
        ui:HGroup{
            ID = "dst",
            ui:Label{
                ID = "ExiftoolPathLabel",
                Text = "Path of Exiftool:"
            },
            ui:TextEdit{
                ID = "ExiftoolPath",
                Text = "",
                PlaceholderText = exiftool_path
            }
        },
        ui:HGroup{
            ID = "buttons",
...
local exif_tags
if get_exif_tags then
    if itm.ExiftoolPath.PlainText ~= nil and itm.ExiftoolPath.PlainText ~= "" then
        -- Use the provided path, otherwise use the default
        exiftool_path = itm.ExiftoolPath.PlainText
    end
    ...
    local cmd = string.format(
        [[%s -csv -s -UniqueCameraModel -ForwardMatrix1 -ForwardMatrix2 -AsShotNeutral -AsShotWhiteXY -ColorMatrix1 -ColorMatrix2 -AnalogBalance -CameraCalibration1 -CameraCalibration2 -CalibrationIlluminant1 -CalibrationIlluminant2 "%s"]],
        exiftool_path, file_path)
    print("Extracting exif tags with command: ", cmd)
    local csv = runCmd(cmd)
    exif_tags = read_csv(csv)
...
for label, _ in pairs(primaries) do
    itm.outputGamut:AddItem(label)
end
itm.outputWB:AddItem('Current WB')
if exif_tags["CalibrationIlluminant1"] ~= nil then
    itm.outputWB:AddItem(exif_tags["CalibrationIlluminant1"])
end
if exif_tags["CalibrationIlluminant2"] ~= nil then
    itm.outputWB:AddItem(exif_tags["CalibrationIlluminant2"])
end
itm.method:AddItem("Use Default")
if exif_tags["ForwardMatrix1"] ~= nil then
    itm.method:AddItem("Use Forward Matrix")
end
...
if get_exif_tags and generate_matrix then
    ...
    local target_primaries = primaries[itm.outputGamut.CurrentText]
    local camera_to_xyz_wp_adapted = multiply(bradford_chromatic_adaptation(D50_XY, target_primaries.white),
        camera_to_pcs)
